...
class Complex;
```
Complex 클래스를 미리 선언한다.
```
class ComplexManager
```
복소수 계산을 담당하는 ComplexManager 클래스를 선언한다.
```
public:
```
외부에서 사용할 수 있는 멤버를 선언하는 접근 지정자이다.
...
```
ComplexManager 클래스를 프렌드 클래스로 선언하여 private 멤버 접근을 허용한다.
```
Complex ComplexManager::ComplexAdd(Complex x, Complex y)
```
ComplexManager 클래스의 ComplexAdd 함수 정의 부분이다.
```
Complex temp;
```
덧셈 결과를 저장할 임시 객체를 생성한다.
...
ComplexManager man;
```
ComplexManager 객체를 생성한다.
```
sum = man.ComplexAdd(x, y);
```
ComplexManager 객체를 이용하여 두 복소수의 합을 계산한다.
```
cout << "두 복소수의 합은 ";
```
출력 안내 문구를 출력한다.
