...
double data[10];
```
최대 10개의 값을 저장할 수 있는 실수형 배열이다.
```
int count;
```
현재 저장된 데이터의 개수를 저장하는 변수이다.
```
public:
```
외부에서 사용할 수 있는 멤버를 선언하는 접근 지정자이다.
...
count = 0;
```
처음에는 저장된 데이터가 없으므로 count를 0으로 초기화한다.
```
...
double getAvg()
```
저장된 데이터의 평균을 구하는 함수이다.
```
double sum = 0;
```
합계를 계산하기 위한 변수를 선언한다.
```
for(int i=0;i<count;i++)
```
저장된 데이터 개수만큼 반복한다.
