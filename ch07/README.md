# 실습과제 1

#  객체의 생성과 소멸의 의미를 설명하라
객체의 생성은 메모리에 객체가 만들어지는 것이고, 소멸은 객체가 사용이 끝나 메모리에서 사라지는 것이다.
# 지역변수(지역객체), 전역변수(전역객체)의 차이를 조사하라, 선언위치, 접근범위, 생성시점, 소멸시점, 초기화 여부 등의 관점에서 비교하라
# 선언 위치

지역객체: 함수 안에서 선언

전역객체: 함수 밖에서 선언

# 접근 범위

지역객체: 해당 함수 안에서만 사용


전역객체: 프로그램 전체에서 사용 가능
생성 시점

지역객체: 함수가 실행될 때 생성

전역객체: 프로그램 시작 시 생성

# 소멸 시점

지역객체: 함수가 끝나면 소멸

전역객체: 프로그램 종료 시 소멸

# 초기화 여부

지역객체: 자동 초기화되지 않음

전역객체: 자동으로 0으로 초기화됨

# 프로그램 실행 후 main함수가 실행되기 전에 어떤 일이 처리되는지 조사해보라
전역 변수와 전역 객체가 먼저 생성되고 초기화된다.
# main함수가 종료 후 프로그램이 종료하기 전에 어떤 일이 처리되는지 조사하라
전역 객체가 소멸되고 프로그램이 종료된다.



# 실습과제 2

# 소스코드 설명
```
#include <iostream>
```
표준 입출력을 사용하기 위해 iostream 헤더파일을 포함한다.

```
using namespace std;
```
std 네임스페이스를 사용하여 std:: 없이 cin, cout 등을 사용한다.

```
class Triangle
```
Triangle 클래스를 선언한다.

```
public:
```
외부에서 접근 가능한 멤버를 선언하기 위한 접근 지정자이다.

```
double s, h;
```
삼각형의 밑변과 높이를 저장하는 실수형 멤버 변수이다.

```
Triangle();
```
매개변수가 없는 기본 생성자를 선언한다.

```
Triangle(double a, double b);
```
밑변과 높이를 받아 초기화하는 생성자를 선언한다.

```
~Triangle();
```
객체가 소멸될 때 실행되는 소멸자를 선언한다.

```
double getArea();
```
삼각형의 면적을 계산하여 반환하는 멤버 함수를 선언한다.

```
Triangle::Triangle() : Triangle(1,1) {}
```
기본 생성자이며 다른 생성자를 호출하는 위임 생성자이다.

```
Triangle::Triangle(double a, double b) : s(a), h(b)
```
매개변수 a와 b를 이용해 s와 h를 초기화하는 생성자이다.

```
cout << "밑변" << s << " 높이" << h << "인 삼각형 생성" << endl;
```
삼각형 객체가 생성되었음을 출력한다.

```
Triangle::~Triangle()
```
Triangle 클래스의 소멸자를 정의하는 부분이다.

```
cout << "밑변" << s << " 높이" << h << "인 삼각형 소멸" << endl;
```
객체가 소멸될 때 메시지를 출력한다.

```
double Triangle::getArea()
```
Triangle 클래스의 멤버 함수 getArea를 정의하는 부분이다.

```
return s * h * 0.5;
```
밑변과 높이를 이용해 삼각형의 면적을 계산하여 반환한다.

```
int main()
```
프로그램이 시작되는 메인 함수이다.

```
Triangle tri1;
```
Triangle 객체 tri1을 생성한다.

```
cout << "삼각형의 면적은 " << tri1.getArea() << endl;
```
tri1의 면적을 계산하여 출력한다.

```
Triangle tri2(2, 4);
```
밑변이 2이고 높이가 4인 Triangle 객체 tri2를 생성한다.

```
cout << "삼각형의 면적은 " << tri2.getArea() << endl;
```
tri2의 면적을 계산하여 출력한다.

```
return 0;
```
프로그램을 정상적으로 종료한다.

# 실행결과
<img width="311" height="174" alt="화면 캡처 2026-03-23 184725" src="https://github.com/user-attachments/assets/4b5bfd31-81df-4e91-bb64-7d2834860297" />


# 실습과제 3

# 소스코드 설명
```
#include <iostream>
```
표준 입출력을 사용하기 위해 iostream 헤더파일을 포함한다.

```
using namespace std;
```
std 네임스페이스를 사용하여 std:: 없이 cin, cout 등을 사용한다.

```
class Triangle
```
Triangle 클래스를 선언한다.

```
public:
```
외부에서 접근 가능한 멤버를 선언하기 위한 접근 지정자이다.

```
double s, h;
```
삼각형의 밑변과 높이를 저장하는 실수형 멤버 변수이다.

```
Triangle();
```
매개변수가 없는 기본 생성자를 선언한다.

```
Triangle(double a, double b);
```
밑변과 높이를 받아 초기화하는 생성자를 선언한다.

```
~Triangle();
```
객체가 소멸될 때 실행되는 소멸자를 선언한다.

```
double getArea();
```
삼각형의 면적을 계산하여 반환하는 멤버 함수를 선언한다.

```
Triangle::Triangle() : Triangle(1, 1) {}
```
기본 생성자이며 다른 생성자를 호출하는 위임 생성자이다.

```
Triangle::Triangle(double a, double b) : s(a), h(b)
```
매개변수 a와 b를 이용해 s와 h를 초기화하는 생성자이다.

```
cout << "밑변" << s << " 높이" << h << "인 삼각형 생성" << endl;
```
삼각형 객체가 생성될 때 메시지를 출력한다.

```
Triangle::~Triangle()
```
Triangle 클래스의 소멸자를 정의하는 부분이다.

```
cout << "밑변" << s << " 높이" << h << "인 삼각형 소멸" << endl;
```
객체가 소멸될 때 메시지를 출력한다.

```
double Triangle::getArea()
```
Triangle 클래스의 멤버 함수 getArea를 정의하는 부분이다.

```
return s * h * 0.5;
```
밑변과 높이를 이용해 삼각형의 면적을 계산하여 반환한다.

```
Triangle tri1(4, 8);
```
전역 객체 tri1을 생성한다. (프로그램 시작 시 생성됨)

```
Triangle tri2(2, 2);
```
전역 객체 tri2를 생성한다. (main 함수 실행 전에 생성됨)

```
int main()
```
프로그램이 시작되는 메인 함수이다.

```
cout << "삼각형의 면적은 " << tri2.getArea() << endl;
```
tri2의 면적을 계산하여 출력한다.

```
cout << "삼각형의 면적은 " << tri1.getArea() << endl;
```
tri1의 면적을 계산하여 출력한다.

```
return 0;
```
프로그램을 정상적으로 종료한다.


# 실행결과
<img width="311" height="163" alt="화면 캡처 2026-03-23 184857" src="https://github.com/user-attachments/assets/ca08a7c8-3d40-48aa-b75e-52de7762221d" />




# 실습과제 4

# 소스코드 설명
```
#include <iostream>
```
표준 입출력을 사용하기 위해 iostream 헤더파일을 포함한다.

```
using namespace std;
```
std 네임스페이스를 사용하여 std:: 없이 cin, cout 등을 사용한다.

```
class Sphere
```
Sphere 클래스를 선언한다.

```
public:
```
외부에서 접근 가능한 멤버를 선언하기 위한 접근 지정자이다.

```
double r;
```
구의 반지름을 저장하는 실수형 멤버 변수이다.

```
Sphere();
```
매개변수가 없는 기본 생성자를 선언한다.

```
Sphere(double a);
```
반지름 값을 받아 초기화하는 생성자를 선언한다.

```
~Sphere();
```
객체가 소멸될 때 실행되는 소멸자를 선언한다.

```
double getVolume();
```
구의 부피를 계산하여 반환하는 멤버 함수를 선언한다.

```
Sphere::Sphere() : Sphere(1) {}
```
기본 생성자이며 다른 생성자를 호출하는 위임 생성자이다.

```
Sphere::Sphere(double a) : r(a)
```
매개변수 a를 이용해 반지름 r을 초기화하는 생성자이다.

```
cout << "반지름" << r << "인 구 생성" << endl;
```
구 객체가 생성될 때 메시지를 출력한다.

```
Sphere::~Sphere()
```
Sphere 클래스의 소멸자를 정의하는 부분이다.

```
cout << "반지름" << r << "인 구 소멸" << endl;
```
객체가 소멸될 때 메시지를 출력한다.

```
double Sphere::getVolume()
```
Sphere 클래스의 멤버 함수 getVolume을 정의하는 부분이다.

```
return (4.0) / (3.0) * 3.14 * r * r * r;
```
반지름을 이용해 구의 부피를 계산하여 반환한다.

```
int main()
```
프로그램이 시작되는 메인 함수이다.

```
Sphere sph1;
```
Sphere 객체 sph1을 생성한다.

```
cout << "구의 부피는 " << sph1.getVolume() << endl;
```
sph1의 부피를 계산하여 출력한다.

```
Sphere sph2(3);
```
반지름이 3인 Sphere 객체 sph2를 생성한다.

```
cout << "구의 부피는 " << sph2.getVolume() << endl;
```
sph2의 부피를 계산하여 출력한다.

```
return 0;
```
프로그램을 정상적으로 종료한다.


# 실행결과
<img width="222" height="152" alt="화면 캡처 2026-03-24 200105" src="https://github.com/user-attachments/assets/e95ba9a0-d170-4503-924c-5d8d372ebf33" />



# 실습과제 5

# 소스코드 설명
```
#include <iostream>
```
표준 입출력을 사용하기 위해 iostream 헤더파일을 포함한다.

```
using namespace std;
```
std 네임스페이스를 사용하여 std:: 없이 cin, cout 등을 사용한다.

```
class Sphere
```
Sphere 클래스를 선언한다.

```
public:
```
외부에서 접근 가능한 멤버를 선언하기 위한 접근 지정자이다.

```
double r;
```
구의 반지름을 저장하는 실수형 멤버 변수이다.

```
Sphere();
```
매개변수가 없는 기본 생성자를 선언한다.

```
Sphere(double a);
```
반지름 값을 받아 초기화하는 생성자를 선언한다.

```
~Sphere();
```
객체가 소멸될 때 실행되는 소멸자를 선언한다.

```
double getVolume();
```
구의 부피를 계산하여 반환하는 멤버 함수를 선언한다.

```
Sphere::Sphere() : Sphere(1) {}
```
기본 생성자이며 다른 생성자를 호출하는 위임 생성자이다.

```
Sphere::Sphere(double a) : r(a)
```
매개변수 a를 이용해 반지름 r을 초기화하는 생성자이다.

```
cout << "반지름" << r << "인 구 생성" << endl;
```
구 객체가 생성될 때 메시지를 출력한다.

```
Sphere::~Sphere()
```
Sphere 클래스의 소멸자를 정의하는 부분이다.

```
cout << "반지름" << r << "인 구 소멸" << endl;
```
객체가 소멸될 때 메시지를 출력한다.

```
double Sphere::getVolume()
```
Sphere 클래스의 멤버 함수 getVolume을 정의하는 부분이다.

```
return (4.0) / (3.0) * 3.14 * r * r * r;
```
반지름을 이용해 구의 부피를 계산하여 반환한다.

```
Sphere sph1(10);
```
전역 객체 sph1을 생성한다. (main 함수 실행 전에 생성된다)

```
Sphere sph2(20);
```
전역 객체 sph2를 생성한다. (main 함수 실행 전에 생성된다)

```
int main()
```
프로그램이 시작되는 메인 함수이다.

```
cout << "구의 부피는 " << sph1.getVolume() << endl;
```
sph1의 부피를 계산하여 출력한다.

```
cout << "구의 부피는 " << sph2.getVolume() << endl;
```
sph2의 부피를 계산하여 출력한다.

```
return 0;
```
프로그램을 정상적으로 종료한다.


# 실행결과
<img width="258" height="166" alt="화면 캡처 2026-03-24 200758" src="https://github.com/user-attachments/assets/505f2e41-0274-4ea2-9ac6-d6428abc8617" />

